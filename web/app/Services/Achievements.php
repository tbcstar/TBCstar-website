...
class Achievements
{
    ...
    private static function LoadAchievements() {
        ...
        self::$achievements_storage = DB::connection('WotlkChar')->table('character_achievement')->where('guid',
            self::$guid)->orderBy('date', 'desc')->get();
        if(!self::$achievements_storage) {
            return false;
        }
        self::$criterias_storage = DB::connection('WotlkChar')->table('character_achievement_progress')->where('guid', self::$guid)->get();
